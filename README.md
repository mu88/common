...
| `image_names` | JSON array of all fully-qualified Docker image references (registry/repository:tag), one per published container family (e.g. chiseled, regular) |
| `digests` | JSON array of all Docker image digests (`sha256:...`) generated by this build. Only populated when `docker-attest-provenance-and-sbom` is `true`. Not keyed to a specific image/tag - consumers publishing a single container family/platform can expect exactly one entry. |
